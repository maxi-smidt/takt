...
`TAKT_REGISTRY_IMAGE` kann `latest`, ein Versionstag wie `0.3.0` oder ein
...
- `ghcr.io/maxi-smidt/takt-registry` wird für `linux/amd64` und
  ...
  Versionstags beispielsweise `0.3.0` sowie `0.3`. Weil jedes Image sein
...
- `compose.yaml`, die Environment-Vorlage und das Unraid-XML werden zusätzlich
  ...
  Version (z. B. `0.3.0`) statt `latest` setzen.
- Ein Tag wie `v0.3.0` muss exakt zur Version in `pyproject.toml` passen und
...
`0.3.0` vergeben und `dist/takt-raspberry-pi.tar.gz` hochladen. Abweichende
